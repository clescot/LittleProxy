...
public final class IdleTest {
  ...
  @Test
  @Disabled(
      "File descriptors vary too much on my laptop, other people saw this problem too: https://github.com/adamfisk/LittleProxy/pull/221")
  public void testFileDescriptorCount() throws Exception {
    ...
    long fileDescriptorsWhileConnectionsOpen =
        TestUtils.getOpenFileDescriptorsAndPrintMemoryUsage();
    Thread.sleep(10000);

    System.gc();
    System.out.println("\n\n------------------ Memory Usage After Idle Timeout ------------------");
    long fileDescriptorsAfterConnectionsClosed =
        TestUtils.getOpenFileDescriptorsAndPrintMemoryUsage();
    ...
    assertThat(fdDeltaRatio)
        .as(
            "Number of file descriptors after close should be much closer to initial value than number of file descriptors while open (+ 1%).\n"
                ...
                + "; file descriptors while connections open: "
                + fileDescriptorsWhileConnectionsOpen
                + "; "
                + "file descriptors after connections closed: "
                + fileDescriptorsAfterConnectionsClosed
                + "\n"
                + "Ratio of file descriptors after connections are closed to descriptors before connections were closed: "
                + fdDeltaRatio)
        .isLessThan(0.01);
  }
}
