...
/**
 * Utility methods for creating HTTP clients and sending requests to servers via a LittleProxy
 * instance.
 */
public class HttpClientUtil {
  public static org.apache.http.HttpResponse performLocalHttpGet(
      int port, String path, HttpProxyServer proxyServer) {
    return performHttpGet("http://localhost:" + port + path, proxyServer);
  }

  /**
   * Creates a new HTTP client that uses the specified LittleProxy instance to perform a GET to the
   * specified URL. The HTTP client is closed and discarded after the request is completed.
   *
   ...
   */
  public static org.apache.http.HttpResponse performHttpGet(
      String url, HttpProxyServer proxyServer) {
  ...
  /**
   * Creates a new HTTP client that uses the specified LittleProxy instance to perform a POST of the
   * specified size to the URL. The POST body will consist of a meaningless UTF-8-encoded String
   * (currently, the letter 'q'). The HTTP client is closed and discarded after the request is
   * completed.
   *
   * @param url URL to post to
  ...
  /**
   * Performs the specified request using the HTTP client. Consumes the response entity and shuts
   * down the HTTP client after the request has been made.
   *
   * @param httpClient HTTP client to use
   * @param request HTTP request to perform
   * @return the HttpResponse from the server
   */
  private static org.apache.http.HttpResponse performHttpRequest(
    ...
    try {
      ...
      // this is a test code; just let all exceptions bubble up the stack, which will cause the test
      // to fail
