...
public class SocketClientUtil {
  ...
  /**
   * Reads all available data from the socket and returns a String containing that content,
   * interpreted in the UTF-8 charset.
   *
  ...
  /**
   * Determines if the socket can be written to. This method tests the writability of the socket by
   * writing to the socket, so it should only be used immediately before closing the socket.
   *
  ...
  /**
   * Determines if the socket can be read from. This method tests the readability of the socket by
   * attempting to read a byte from the socket. If successful, the byte will be lost, so this method
   * should only be called immediately before closing the socket.
   *
   * @param socket socket to test
  ...
  public static boolean isSocketReadyToRead(Socket socket) throws IOException {
    ...
    try {
      ...
      // the socket couldn't be read, perhaps because the connection was reset or some other error.
      // it cannot be read.
      return false;
  ...
  /**
   * Opens a socket to the specified proxy server with a 3s timeout. The socket should be closed
  ...
  public static Socket getSocketToProxyServer(HttpProxyServer proxyServer) throws IOException {
    ...
    socket.connect(
        new InetSocketAddress("localhost", proxyServer.getListenAddress().getPort()), 1000);
