...
public final class HttpFilterTest {
  ...
  /**
   * Sets up the HttpProxyServer instance for a test. This method initializes the proxyServer and
   * proxyPort method variables, and should be called before making any requests through the proxy
   * server.
   *
   * <p>The proxy cannot be created in @BeforeEach method because the filtersSource must be
   * initialized by each test before the proxy is created.
   *
   * @param filtersSource HTTP filters source
   */
  private void setUpHttpProxyServer(@NonNull HttpFiltersSource filtersSource) {
    proxyServer =
  ...
  public void testFiltering() throws Exception {
    ...
    final AtomicLongArray proxyToServerRequestSendingNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray proxyToServerRequestSentNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray serverToProxyResponseReceivingNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray serverToProxyResponseReceivedNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray proxyToServerConnectionQueuedNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray proxyToServerResolutionStartedNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray proxyToServerResolutionSucceededNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray proxyToServerResolutionFailedNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray proxyToServerConnectionStartedNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray proxyToServerConnectionSSLHandshakeStartedNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray proxyToServerConnectionFailedNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray proxyToServerConnectionSucceededNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    final AtomicLongArray serverToProxyResponseTimedOutNanos =
        new AtomicLongArray(new long[] {-1, -1, -1, -1, -1});
    ...
    assertThat(proxyToServerConnectionQueuedNanos.get(i))
        .isLessThan(proxyToServerResolutionStartedNanos.get(i));
    assertThat(proxyToServerResolutionStartedNanos.get(i))
        .isLessThan(proxyToServerResolutionSucceededNanos.get(i));
    ...
    assertThat(proxyToServerConnectionSucceededNanos.get(i))
    ...
    InetSocketAddress remoteAddress =
        (InetSocketAddress) serverCtxReference.get().channel().remoteAddress();
    ...
    assertThat(remoteAddress.getHostName())
        .as("Server's remoteAddress should connect to localhost")
        .isEqualTo("localhost");
    assertThat(remoteAddress.getPort())
        .as("Server's port should match the web server port")
        .isEqualTo(webServerPort);
  ...
  public void testResolutionStartedFilterReturnsUnresolvedAddress() throws Exception {
          ...
          public HttpFilters filterRequest(@NonNull HttpRequest originalRequest) {
            return new HttpFiltersAdapter(originalRequest) {
              ...
              public void proxyToServerResolutionSucceeded(
                  @NonNull String serverHostAndPort,
                  @NonNull InetSocketAddress resolvedRemoteAddress) {
                assertThat(resolvedRemoteAddress.isUnresolved())
                    .as("expected to receive a resolved InetSocketAddress")
                    .isFalse();
    ...
    assertThat(resolutionSucceeded.get())
        .as("proxyToServerResolutionSucceeded method was not called")
        .isTrue();
  }
  ...
  public void testResolutionFailedCalledAfterDnsFailure() throws Exception {
    ...
    when(mockFailingResolver.resolve("www.does-not-exist", 80))
        .thenThrow(new UnknownHostException("www.does-not-exist"));
    ...
    // invoked/not invoked as appropriate, but also verify that
    // other filters were invoked/not invoked as expected
    assertThat(filter.isProxyToServerResolutionSucceededInvoked())
        .as("proxyToServerResolutionSucceeded method was called but should not have been")
        .isFalse();
    assertThat(filter.isProxyToServerResolutionFailedInvoked())
        .as("proxyToServerResolutionFailed method was not called")
        .isTrue();

    assertThat(filter.isClientToProxyRequestInvoked())
        .as("Expected filter method to be called")
        .isTrue();
    assertThat(filter.isProxyToServerConnectionQueuedInvoked())
        .as("Expected filter method to be called")
        .isTrue();
    assertThat(filter.isProxyToServerResolutionStartedInvoked())
        .as("Expected filter method to be called")
        .isTrue();
    assertThat(filter.isProxyToClientResponseInvoked())
        .as("Expected filter method to be called")
        .isTrue();

    assertThat(filter.isProxyToServerConnectionStartedInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isProxyToServerRequestInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isProxyToServerAllowMitmInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isProxyToServerConnectionFailedInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isProxyToServerConnectionSucceededInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isProxyToServerRequestSendingInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isProxyToServerRequestSentInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isProxyToServerConnectionSSLHandshakeStartedInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isServerToProxyResponseReceivingInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isServerToProxyResponseInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isServerToProxyResponseReceivedInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
    assertThat(filter.isServerToProxyResponseTimedOutInvoked())
        .as("Expected filter method to not be called")
        .isFalse();
  }
  ...
  public void testConnectionFailedCalledAfterConnectionFailure() throws Exception {
    ...
    // other filters were invoked/not invoked as expected
    assertThat(filter.isProxyToServerConnectionSucceededInvoked())
        .as("proxyToServerConnectionSucceeded should not be called when connection fails")
        .isFalse();
    assertThat(filter.isProxyToServerConnectionFailedInvoked())
        .as("proxyToServerConnectionFailed should be called when connection fails")
        .isTrue();
  ...
  /**
   * Verifies the proper filters are invoked when an attempt to connect to an unencrypted upstream
   * chained proxy fails.
   */
  @Test
  public void testFiltersAfterUnencryptedConnectionToUpstreamProxyFails() throws Exception {
    ...
    assertThat(filter.isProxyToServerConnectionSucceededInvoked())
        .as(
            "proxyToServerConnectionSucceeded should not be called when connection to chained proxy fails")
        .isFalse();
    assertThat(filter.isProxyToServerConnectionFailedInvoked())
        .as("proxyToServerConnectionFailed should be called when connection to chained proxy fails")
        .isTrue();
  ...
  /**
   * Verifies the proper filters are invoked when an attempt to connect to an upstream chained proxy
   * over SSL fails. (The proxyToServerConnectionFailed() filter method is particularly important.)
   */
  @Test
  public void testFiltersAfterSSLConnectionToUpstreamProxyFails() throws Exception {
    ...
    // proxy tests
    final HttpProxyServer chainedProxy =
        DefaultHttpProxyServer.bootstrap()
  ...
  public void testResponseTimedOutInvokedAfterServerTimeout() throws Exception {
    ...
    org.apache.http.HttpResponse httpResponse =
        performLocalHttpGet(mockServerPort, "/servertimeout", proxyServer);
  ...
  public void testRequestSentInvokedAfterLastHttpContentSent() throws Exception {
            ...
            return new HttpFiltersAdapter(originalRequest) {
              ...
              public HttpResponse proxyToServerRequest(@NonNull HttpObject httpObject) {
                if (httpObject instanceof LastHttpContent) {
                  assertThat(requestSentCallbackInvoked.get())
                      .as(
              ...
              public void proxyToServerRequestSent() {
                ...
                // including payload, has been sent to the server
                assertThat(lastHttpContentProcessed.get())
                    .as(
    ...
    assertThat(lastHttpContentProcessed.get())
        .as("proxyToServerRequest callback was not invoked for LastHttpContent for chunked POST")
        .isTrue();
    assertThat(requestSentCallbackInvoked.get())
        .as("proxyToServerRequestSent callback was not invoked for chunked POST")
        .isTrue();
    ...
    assertThat(lastHttpContentProcessed.get())
        .as("proxyToServerRequest callback was not invoked for LastHttpContent for GET")
        .isTrue();
    assertThat(requestSentCallbackInvoked.get())
        .as("proxyToServerRequestSent callback was not invoked for GET")
        .isTrue();
  }

  /**
   * Verifies that the proxy properly handles a null HttpFilters instance, as allowed in the {@link
   * HttpFiltersSource#filterRequest(HttpRequest, ChannelHandlerContext)} documentation.
   */
  @Test
  public void testNullHttpFilterSource() throws Exception {
    mockServer.stubFor(
        get(urlEqualTo("/testNullHttpFilterSource"))
    ...
    org.apache.http.HttpResponse httpResponse =
        performLocalHttpGet(mockServerPort, "/testNullHttpFilterSource", proxyServer);
    Thread.sleep(500);
  ...
  private long now() {
    ...
    // that any two callbacks would be invoked in the same nanosecond,
    // even on very fast hardware
    return System.nanoTime();
  }

  /**
   * HttpFilters instance that monitors HttpFilters methods and tracks which methods have been
   * invoked.
   */
  private static class HttpFiltersMethodInvokedAdapter implements HttpFilters {
    ...
    private final AtomicBoolean proxyToServerConnectionSSLHandshakeStarted =
