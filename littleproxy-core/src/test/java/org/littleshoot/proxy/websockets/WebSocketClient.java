...
/** Simple WebSocket client for use in unit tests that sends and receives text frames. */
public class WebSocketClient {
  ...
  public void open(
      final URI uri, final Duration connectTimeout, final Optional<InetSocketAddress> httpProxy)
      throws TimeoutException {
    logger.info(
        "{} connecting to {} via proxy {}",
        getClass().getSimpleName(),
        uri,
        httpProxy.map(Object::toString).orElse("(none)"));
    ...
    try {
      ...
      final WebSocketFrameReader handler =
      ...
      if (httpProxy.isPresent() && !isSecure) {
        connectFuture =
            bootstrap.connect(httpProxy.get().getHostString(), httpProxy.get().getPort());
      } else {
  ...
  private static class WebSocketClientChannelInitializer extends ChannelInitializer<SocketChannel> {
    ...
    public WebSocketClientChannelInitializer(
    ...
    public void initChannel(final SocketChannel ch) throws Exception {
      ...
      if (SECURE_SCHEMES.contains(uri.getScheme())) {
        ...
        final SslHandler sslHandler =
            sslContext.newHandler(ch.alloc(), uri.getHost(), uri.getPort());
