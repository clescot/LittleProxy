...
public final class TimeoutTest {
  ...
  public void testIdleConnectionTimeout() throws Exception {
    proxyServer =
        DefaultHttpProxyServer.bootstrap().withPort(0).withIdleConnectionTimeout(1).start();

    mockServer.stubFor(
        get(urlEqualTo("/idleconnection"))
    ...
    try (CloseableHttpClient httpClient =
      ...
      assertThat(response.getStatusLine().getStatusCode())
          .as(
              "Expected to receive an HTTP 504 (Gateway Timeout) response after proxy did not receive a response within 1 second")
          .isEqualTo(504);
  ...
  public void testConnectionTimeout() throws Exception {
    ...
    try (CloseableHttpClient httpClient =
      ...
      assertThat(response.getStatusLine().getStatusCode())
          .as(
              "Expected to receive an HTTP 502 (Bad Gateway) response after proxy could not connect within 1 second")
          .isEqualTo(502);
      assertThat(MILLISECONDS.convert(stop - start, TimeUnit.NANOSECONDS))
  ...
  /**
   * Verifies that when the client times out sending the initial request, the proxy still returns a
   * Gateway Timeout.
   */
  @Test
  public void testClientIdleBeforeRequestReceived() throws IOException, InterruptedException {
    ...
    // connect to the proxy and begin to transmit the request, but don't send the
    // trailing \r\n that indicates the client has completely transmitted the
    // request
    String successfulGet = "GET http://localhost:" + mockServerPort + "/success HTTP/1.1";

    // using the SocketClientUtil since we want the client to fail to send the
    // entire GET request, which is not possible with
