  ...
  public void testProxyKeepsClientConnectionOpenAfterServerDisconnect()
    ...
    for (int i = 1; i <= 2; i++) {
      ...
      assertThat(response)
          .as("Expected to receive an HTTP 200 from the server (iteration: %s)", i)
          .startsWith("HTTP/1.1 200 OK");
      // the proxy will set the Transfer-Encoding to chunked since the server is using
      // connection closure to indicate the end of the message.
      // (matching capitalized or lowercase Transfer-Encoding, since Netty 4.1+ uses
