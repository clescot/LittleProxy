...
public class ProxyUtils {
  /**
   * Hop-by-hop headers that should be removed when proxying, as defined by the (<a
   * href="https://www.w3.org/Protocols/rfc2616/rfc2616-sec13.html#sec13.5.1">HTTP 1.1 spec, section
   * 13.5.1</a>). Transfer-Encoding is NOT included in this list, since LittleProxy does not
   * typically modify the transfer encoding. See also {@link #shouldRemoveHopByHopHeader(String)}.
   *
   * <p>Header names are stored as lowercase to make case-insensitive comparisons easier.
   */
  @SuppressWarnings(
      "deprecation") // Don't remove header names from this set until they're removed from Netty,
  // just in case someone's still using them.
  ...
  /**
   * If an HttpObject implements the market interface LastHttpContent, it represents the last chunk
   * of a transfer.
   *
  ...
  /**
   * If an HttpObject is not the last chunk, then that means there are other chunks that will
   * follow.
   *
  ...
  /**
   * Adds the Via header to specify that the message has passed through the proxy. The specified
   * alias will be appended to the Via header line. The alias may be the hostname of the machine
   * proxying the request, or a pseudonym. From RFC 7230, section 5.7.1:
   *
   * <pre>
  ...
  public static void addVia(HttpMessage httpMessage, String alias) {
    String newViaHeader =
        String.valueOf(httpMessage.protocolVersion().majorVersion())
            + '.'
   ...
   * @return <code>true</code> if the specified string is either "true" or "on" ignoring case,
   ...
   *
   * @param val The string in question.
   * @return <code>true</code> if the specified string is either "false" or "off" ignoring case,
   *     otherwise <code>false</code>.
   */
   ...
   * This code is taken directly from {@link
   * io.netty.handler.codec.http.HttpObjectDecoder#isContentAlwaysEmpty(HttpMessage)}.
   *
   * @param msg HTTP message
   * @return true if the HTTP message is always empty, false if the message <i>may</i> have entity
   *     content.
   */
  ...
  /**
   * Returns true if the HTTP response from the server is expected to indicate its own message
   * length/end-of-message. Returns false if the server is expected to indicate the end of the HTTP
   * entity by closing the connection.
   *
   * <p>This method is based on the allowed message length indicators in the HTTP specification,
   * section 4.4:
   *
   * <pre>
   * 4.4 Message Length
   ...
   * @return true if the message will indicate its own message length, or false if the server is
   *     expected to indicate the message length by closing the connection
   */
  public static boolean isResponseSelfTerminating(HttpResponse response) {
    ...
    // if there is a Transfer-Encoding value, determine whether the final encoding is "chunked",
    // which makes the message self-terminating
    List<String> allTransferEncodingHeaders =
        getAllCommaSeparatedHeaderValues(HttpHeaderNames.TRANSFER_ENCODING, response);
    if (!allTransferEncodingHeaders.isEmpty()) {
      ...
      // per #3 above: "If a message is received with both a Transfer-Encoding header field and a
      // Content-Length header field, the latter MUST be ignored."
      // since the Transfer-Encoding field is present, the message is self-terminating if and only
      // if the final Transfer-Encoding value is "chunked"
    ...
    // not checking for multipart/byteranges, since it is seldom used and its use as a message
    // length indicator was removed in RFC 7230

    // none of the other message length indicators are present, so the only way the server can
    // indicate the end
   ...
   * Any whitespace (spaces or tabs) surrounding the values will be removed. Empty values (e.g. two
   * consecutive commas, or a value followed by a comma and no other value) will be removed; they
   * will not appear as empty elements in the returned list. If the message contains repeated
   * headers, their values will be added to the returned list in the order in which the headers
   ...
   *
   * <p>Placing values on multiple header lines is allowed under certain circumstances in RFC 2616
   * section 4.2, and in RFC 7230 section 3.2.2 quoted here:
   *
   * <pre>
   ...
   * @return a list of single header values, or an empty list if the header was not present in the
   *     message or contained no values
   */
  public static List<String> getAllCommaSeparatedHeaderValues(
      AsciiString headerName, HttpMessage httpMessage) {
   ...
   * content associated with that response.
   *
   ...
   * From <a href="https://www.w3.org/Protocols/rfc2616/rfc2616-sec13.html#sec13.5.1">section
   * 13.5.1</a> :
   *
   * <pre>
   ...
   *
   * <p>For example, if the value of the Connection header is "Transfer-Encoding, close", this
   * method will return "Transfer-Encoding" and "close".
   *
   * <p>This method strips trims any optional whitespace from the tokens. Unlike {@link
   * #getAllCommaSeparatedHeaderValues(AsciiString, HttpMessage)}, this method only operates on a
   * single header value, rather than all instances of the header in a message.
   *
  ...
  /**
   * Creates a new {@link FullHttpResponse} with the specified String as the body contents (encoded
   * using UTF-8).
   *
  ...
  public static FullHttpResponse createFullHttpResponse(
      HttpVersion httpVersion, HttpResponseStatus status, String body) {
    byte[] bytes = body.getBytes(StandardCharsets.UTF_8);
    ByteBuf content = Unpooled.copiedBuffer(bytes);

    return createFullHttpResponse(
        httpVersion, status, "text/html; charset=utf-8", content, bytes.length);
  }
   ...
   */
  public static FullHttpResponse createFullHttpResponse(
      HttpVersion httpVersion, HttpResponseStatus status) {
   ...
   * @param contentLength number of bytes to send in the Content-Length header; should equal the
   *     number of bytes in the ByteBuf
   * @return new http response object
   */
  public static FullHttpResponse createFullHttpResponse(
  ...
  /**
   * Given an HttpHeaders instance, removes 'sdch' from the 'Accept-Encoding' header list (if it
   * exists) and returns the modified instance.
   *
   * <p>Removes all occurrences of 'sdch' from the 'Accept-Encoding' header.
   *
   * @param headers The headers to modify.
   */
  ...
  /**
   * Tests whether the given response indicates that the connection is switching to the WebSocket
   * protocol.
   *
  ...
  /**
   * Tests whether the given request indicates that the connection is switching to the WebSocket
   * protocol.
   *
