...
/**
 * Manages thread pools for one or more proxy server instances. When servers are created, they must
 * register with the ServerGroup using {@link #registerProxyServer(HttpProxyServer)}, and when they
 * shut down, must unregister with the ServerGroup using {@link
...
public class ServerGroup {
   ...
   * by worker threads, not acceptor threads.)
   */
  ...
  /**
   * The ID of this server group. Forms part of the name of each thread created for this server
   * group. Useful for differentiating threads when multiple proxy instances are running.
   */
  ...
  /**
   * List of all servers registered to use this ServerGroup. Any access to this list should be
   * synchronized using the {@link #SERVER_REGISTRATION_LOCK}.
   */
  public final Set<HttpProxyServer> registeredServers = new HashSet<>(1);

  /**
   * A mapping of {@link TransportProtocol}s to their initialized {@link ProxyThreadPools}. Each
   * transport uses a different thread pool, since the initialization parameters are different.
   */
  private final EnumMap<TransportProtocol, ProxyThreadPools> protocolThreadPools =
      new EnumMap<>(TransportProtocol.class);

  /**
   * A mapping of selector providers to transport protocols. Avoids special-casing each transport
   * protocol during transport protocol initialization.
   */
  private static final EnumMap<TransportProtocol, SelectorProvider>
      TRANSPORT_PROTOCOL_SELECTOR_PROVIDERS = new EnumMap<>(TransportProtocol.class);
  ...
  /**
   * Retrieves the {@link ProxyThreadPools} for the specified transport protocol. Lazily initializes
   * the thread pools for the transport protocol if they have not yet been initialized. If the
   * protocol has already been initialized, this method returns immediately, without
   * synchronization. If initialization is necessary, the initialization process creates the
   * acceptor and worker threads necessary to service requests to/from the proxy.
   *
   * <p>This method is thread-safe; no external locking is necessary.
   *
    ...
    if (protocolThreadPools.get(protocol) == null) {
      synchronized (THREAD_POOL_INIT_LOCK) {
        if (protocolThreadPools.get(protocol) == null) {
          ...
          ProxyThreadPools threadPools =
              new ProxyThreadPools(
                  selectorProvider,
                  incomingAcceptorThreads,
                  incomingWorkerThreads,
                  outgoingWorkerThreads,
  ...
  /**
   * Lock controlling access to the {@link #registerProxyServer(HttpProxyServer)} and {@link
   * #unregisterProxyServer(HttpProxyServer, boolean)} methods.
   */
