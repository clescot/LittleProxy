 ...
 *
 * <p>Once a ProxyToServerConnection has been created for a given server, it is continually reused.
 ...
 *
 * <p>As the ProxyToServerConnections receive responses from their servers, they feed these back to
...
public class ClientToProxyConnection extends ProxyConnection<HttpRequest> {
   ...
   *
   * <p>If we don't yet have a {@link ProxyToServerConnection} for the desired server, this takes
   * care of creating it.
   *
   * <p>Note - the "server" could be a chained proxy, not the final endpoint for the request.
   */
  ...
  final ConnectionFlowStep<HttpRequest> RespondCONNECTSuccessful =
   ...
   *
   * <p>After failing to connect to the server, one of two things can happen:
   *
   * <ol>
   *   <li>If the server was a chained proxy, we fall back to connecting to the ultimate endpoint
   *       directly.
   *   <li>If the server was the ultimate endpoint, we return a 502 Bad Gateway to the client.
   * </ol>
   *
   ...
   *
   * <p>If the request contains credentials, these are checked.
   *
   * <p>If authentication is still required, either because no credentials were provided or the
