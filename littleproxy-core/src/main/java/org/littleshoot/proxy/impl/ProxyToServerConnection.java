...
public class ProxyToServerConnection extends ProxyConnection<HttpResponse> {
  ...
  @Nullable private volatile AddressResolverGroup<?> remoteAddressResolver;
  @Nullable private volatile String username;
  @Nullable private volatile String password;
  private final String serverHostAndPort;
  @Nullable private volatile ChainedProxy chainedProxy;
   ...
   * configured.
   */
  @Nullable private volatile ConnectionFlow connectionFlow;
   ...
   */
  @Nullable private volatile HttpRequest initialRequest;

  /**
   * Keeps track of HttpRequests that have been issued so that we can associate them with responses
   * that we get back
   */
  @Nullable private volatile HttpRequest currentHttpRequest;
  ...
  ConnectionState readHTTPInitial(HttpResponse httpResponse) {
    ...
    if (httpResponse.decoderResult().isFailure()) {
      LOG.debug(
          "Could not parse response from server. Decoder result: {}",
          httpResponse.decoderResult().toString());

      // create a "substitute" Bad Gateway response from the server, since we couldn't understand
      // what the actual
      // response from the server was. set the keep-alive on the substitute response to false so the
      // proxy closes
  ...
  /**
   * Responses to HEAD requests aren't supposed to have content, but Netty doesn't know that any
   * given response is to a HEAD request, so it needs to be told that there's no content so that it
  ...
  private class HeadAwareHttpResponseDecoder extends HttpResponseDecoder {
    ...
    public HeadAwareHttpResponseDecoder(
  ...
  ChannelFuture write(Object msg) {
    ...
    if (is(DISCONNECTED) && msg instanceof HttpRequest) {
      ...
      // only write this message if a connection was established and is not in the process of
      // disconnecting or
      ...
      if (isConnecting() || getCurrentState().isDisconnectingOrDisconnected()) {
        LOG.debug(
            ...
            msg);
        if (msg instanceof ReferenceCounted) {
          // fix: when connection was disconnecting or disconnected, retain() the refCnt = 2, can
          // not release the msg , final leading to OutOfDirectMemoryError.
  ...
  protected void exceptionCaught(Throwable cause) {
    try {
      if (cause instanceof ProxyConnectException) {
        ...
        // IOExceptions are expected errors, for example when a server drops the connection. rather
        // than flood
        // the logs with stack traces for these expected exceptions, log the message at the INFO
        // level and the
  ...
  /**
   * This method initializes our {@link ConnectionFlow} based on however this connection has been
   * configured. If the {@link #disableSni} value is true, this method will not pass peer
  ...
  private final ConnectionFlowStep<HttpResponse> ConnectChannel =
  ...
  private final ConnectionFlowStep<HttpResponse> HTTPCONNECTWithChainedProxy =
  ...
  private final ConnectionFlowStep<HttpResponse> SOCKS4CONNECTWithChainedProxy =
      new ConnectionFlowStep<>(this, AWAITING_CONNECT_OK) {
        ...
        protected Future<?> execute() {
          ...
          DefaultSocks4CommandRequest connectRequest =
              new DefaultSocks4CommandRequest(
  ...
  private final ConnectionFlowStep<HttpResponse> SOCKS5InitialRequest =
      new ConnectionFlowStep<>(this, AWAITING_CONNECT_OK) {
  ...
  private final ConnectionFlowStep<HttpResponse> SOCKS5SendPasswordCredentials =
      new ConnectionFlowStep<>(this, AWAITING_CONNECT_OK) {
  ...
  /**
   * Establishes a SOCKS5 connection after {@link #SOCKS5InitialRequest} and (optionally) {@link
   * #SOCKS5SendPasswordCredentials} have completed.
   */
  private final ConnectionFlowStep<HttpResponse> SOCKS5CONNECTRequestWithChainedProxy =
      new ConnectionFlowStep<>(this, AWAITING_CONNECT_OK) {

        @Override
        protected Future<?> execute() {
          InetSocketAddress destinationAddress = unresolvedAddressFor(serverHostAndPort);
          DefaultSocks5CommandRequest connectRequest =
   ...
   *
   * <p>This does not wait for the handshake to finish so that we can go on and respond to the
   * CONNECT request.
   */
  private final ConnectionFlowStep<HttpResponse> MitmEncryptClientChannel =
      new ConnectionFlowStep<>(this, HANDSHAKING) {
        @Override
        boolean shouldExecuteOnEventLoop() {
          return false;
        }
  ...
  /**
   * Called when the connection to the server or upstream chained proxy fails. This method may
   * return true to indicate that the connection should be retried. If returning true, this method
   * must set up the connection itself.
   *
  ...
  protected boolean connectionFailed(Throwable cause) throws UnknownHostException {
    // unlike a browser, java throws an exception when receiving an unrecognized_name TLS warning,
    // even if the server
    ...
    if (!disableSni && (cause instanceof SSLProtocolException)
      ...
      // unfortunately java does not expose the specific TLS alert number (112), so we have to look
      // for the
  ...
  /**
   * Convenience method to prepare to retry this connection. Closes the connection's channel and
   * sets up the connection again using {@link #setupConnectionParameters()}.
   *
   * @throws UnknownHostException when {@link #setupConnectionParameters()} is unable to resolve the
   *     hostname
   */
    ...
    if (chainedProxy != null && chainedProxy != ChainedProxyAdapter.FALLBACK_TO_DIRECT_CONNECTION) {
      ...
      try {
        if (remoteAddress == null) {
          ...
          hostAndPort =
              HostAndPort.fromParts(remoteAddress.getHostName(), remoteAddress.getPort())
                  .toString();
          remoteAddress =
              proxyServer
                  .getServerResolver()
                  .resolve(remoteAddress.getHostName(), remoteAddress.getPort());
        }
      } catch (UnknownHostException e) {
  ...
  private void initChannelPipeline(ChannelPipeline pipeline) {
    ...
    pipeline.addLast(
        HTTP_DECODER_NAME,
        new HeadAwareHttpResponseDecoder(
   ...
   *
   * @param shouldForwardInitialRequest whether we should forward the initial HttpRequest to the
   *     server after the connection has been established.
   */
  void connectionSucceeded(boolean shouldForwardInitialRequest) {
    ...
    // we're now done with the initialRequest: it's either been forwarded to the upstream server
    // (HTTP requests), or
    // completely dropped (HTTPS CONNECTs). if the initialRequest is reference counted (typically
  ...
  /**
   * Similar to {@link #addressFor(String, DefaultHttpProxyServer)} except that it does not resolve
   * the address.
   *
  ...
  void switchToWebSocketProtocol() {
    final List<String> orderedHandlersToRemove =
        Arrays.asList(
  ...
  private final RequestWrittenMonitor requestWrittenMonitor =
      new RequestWrittenMonitor() {
        @Override
