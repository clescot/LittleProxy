 ...
 *   <li>serverToProxyResponse (can be multiple if chunked)
...
public interface HttpFilters {
   ...
   * client to determine the length of the message (see <a
   * href="https://tools.ietf.org/html/rfc7230#section-3.3.3">RFC 7230, section 3.3.3</a> ).
   *
   * <p>For messages that may contain a body, you may do this by setting the Transfer-Encoding to
   ...
   * <p><b>Important:</b> When returning a response, you must include a mechanism to allow the
   * client to determine the length of the message (see <a
   * href="https://tools.ietf.org/html/rfc7230#section-3.3.3">RFC 7230, section 3.3.3</a> ). For
   ...
   * "stalls").
   *
   * <p>See {@link HttpProxyServerBootstrap#withIdleConnectionTimeout(int)} for information on
   * setting the timeout.
   */
   ...
   *
   * <p>Has no impact if man in the middle is not enabled.
   *
